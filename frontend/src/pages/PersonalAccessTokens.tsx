...
function OneTimeTokenNotice({ token, onDismiss }: OneTimeTokenNoticeProps) {
  ...
  return (
    <div
      className="mb-6 rounded-lg border border-amber-300 bg-amber-50 p-4 shadow-sm dark:border-amber-800 dark:bg-amber-950"
      role="alert"
    >
      <h3 className="text-lg font-semibold text-amber-950 dark:text-amber-100">Copy your new token now</h3>
      ...
      <div className="mt-3 rounded-md border border-amber-200 bg-white p-3 font-mono text-sm text-gray-900 break-all dark:border-amber-800 dark:bg-gray-900 dark:text-gray-100">
        {token}
      </div>
      <div className="mt-3 rounded-md border border-amber-200 bg-white p-3 text-sm text-amber-950 dark:border-amber-800 dark:bg-gray-900 dark:text-amber-100">
        ...
        <p className="mt-1 font-mono text-xs text-amber-900 dark:text-amber-200">
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button
          type="button"
          onClick={onDismiss}
          className="rounded-md border border-amber-300 bg-white px-3 py-1.5 text-sm font-medium text-amber-900 hover:bg-amber-100 dark:border-amber-700 dark:bg-gray-800 dark:text-amber-100 dark:hover:bg-gray-700"
        >
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <h3 className="font-medium text-gray-900 dark:text-gray-100">{token.name}</h3>
          <div className="mt-2 flex flex-wrap gap-2 text-xs">
            <span className="rounded-full bg-blue-50 px-2 py-1 font-medium text-blue-700">
              {scopeLabel(token.scopes)}
            </span>
            {token.scopes.map((scope) => (
              <span key={scope} className="rounded-full bg-gray-100 px-2 py-1 text-gray-600 dark:bg-gray-800 dark:text-gray-400">
                {scope}
              </span>
            ))}
          </div>
          <div className="mt-3 grid gap-1 text-sm text-gray-600 sm:grid-cols-3 dark:text-gray-400">
...
export default function PersonalAccessTokens() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Personal access tokens</h2>
      ...
      {isCreating ? (
        <div className="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Create Personal Access Token</h3>
          <form onSubmit={handleCreate}>
            <div className="mb-4">
              <label htmlFor="pat-name" className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
            ...
            <fieldset className="mb-4">
              <legend className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Scopes</legend>
              <div className="space-y-2">
                <label className="flex items-start gap-2 rounded-md border border-gray-200 p-3 text-sm dark:border-gray-700">
                  <input
                    type="radio"
                    name="pat-scopes"
                    value="read"
                    ...
                    className="mt-1 text-primary-600 focus:ring-primary-500 dark:bg-gray-900 dark:text-primary-400"
                  />
                  <span>
                    <span className="block font-medium text-gray-900 dark:text-gray-100">Read only</span>
                    <span className="text-gray-500 dark:text-gray-400">Allows MCP clients to read Cockpit data.</span>
                  </span>
                </label>
                <label className="flex items-start gap-2 rounded-md border border-gray-200 p-3 text-sm dark:border-gray-700">
                  <input
                  ...
                  />
                  <span>
                    <span className="block font-medium text-gray-900 dark:text-gray-100">Read and write</span>
            ...
            <div className="mb-4">
              <label htmlFor="pat-expires" className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
                Expires (optional)
              </label>
              <input
                id="pat-expires"
                ...
                className="rounded-md border border-gray-300 p-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 dark:placeholder-gray-500"
              />
            </div>
      ...
      {!isLoading && tokens && tokens.length === 0 && !isCreating && (
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <p className="mb-2 text-gray-500 dark:text-gray-400">No personal access tokens yet.</p>
