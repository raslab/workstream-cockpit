...
export function TagAutocomplete({ value, onChange, onTagSelect, textareaRef }: TagAutocompleteProps) {
  ...
  return (
    <div
      ref={dropdownRef}
      className="fixed z-50 w-64 rounded-md border border-gray-200 bg-white shadow-lg max-h-48 overflow-y-auto"
      style={{
        top: `${position.top}px`,
      ...
      {filteredTags.map((tag, index) => (
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-gray-500">ID: #{tag.name}</div>
          </div>
        </button>
      ))}
    </div>
