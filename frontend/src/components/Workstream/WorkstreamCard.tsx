...
export function WorkstreamCard({ workstream }: WorkstreamCardProps) {
  ...
  return (
    <>
      <article
        className={`relative grid grid-cols-[7px_66px_minmax(0,1fr)_auto] gap-x-3 rounded-lg border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-gray-800 ${workstream.state === 'closed' ? 'bg-gray-50/80 dark:bg-gray-800/70' : ''}`}
        style={categoryStyle}
      >
        <div
          data-testid="workstream-category-rail"
          className="col-start-1 row-span-5 rounded-l-lg bg-[var(--workstream-category)]"
          style={{ backgroundColor: categoryColor }}
        />

        <div className="col-start-2 row-span-5 border-r border-gray-900/[0.03] bg-[var(--workstream-category-soft)] dark:border-gray-700/40 dark:bg-gray-900/30" />

        <div
          data-testid="workstream-category-icon"
          className="relative col-start-2 row-start-1 mx-auto mt-4 flex h-11 w-11 items-center justify-center rounded-lg text-2xl shadow-[inset_0_0_0_1px_rgba(15,23,42,0.07)]"
        ...
        <Link
          ...
          className="col-start-3 row-start-1 min-w-0 pt-4 hover:text-primary-600 dark:hover:text-primary-400"
        >
          <h3 className={`text-lg font-bold leading-tight text-gray-900 dark:text-gray-100 ${workstream.state === 'closed' ? 'text-gray-600 dark:text-gray-400' : ''}`}>
            {name}
          </h3>
        </Link>

        <button
          className="col-start-4 row-start-1 mr-4 mt-4 inline-flex h-8 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-gray-300 bg-white px-3 text-xs font-bold text-gray-800 shadow-sm hover:border-gray-400 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700"
        ...
        {workstream.parent && (
          <Link
            ...
            className="col-start-3 col-end-5 row-start-2 mt-1 flex min-w-0 items-center gap-1.5 overflow-hidden pr-4 text-xs text-gray-500 hover:text-primary-600 dark:text-gray-400 dark:hover:text-primary-400"
          >
            <ParentIcon />
        ...
        <Link
          to={`/workstreams/${workstream.id}`}
          className={`col-start-3 col-end-4 min-w-0 pr-2 ${workstream.parent ? 'row-start-3 mt-2' : 'row-start-2 mt-2'}`}
        >
          {latestStatus ? (
            <MarkdownRenderer content={latestStatus.status} className="text-[15px] leading-relaxed text-gray-700 dark:text-gray-300" />
          ) : (
            <p className="text-[15px] leading-relaxed text-gray-500 dark:text-gray-400">No status updates yet</p>
          )}
        </Link>

        <div className={`col-start-3 col-end-5 flex min-w-0 flex-wrap items-center gap-2 pr-4 ${workstream.parent ? 'row-start-4 mt-2' : 'row-start-3 mt-2'}`}>
        ...
        <div className={`col-start-3 col-end-5 row-start-5 flex min-w-0 items-center justify-between gap-3 pb-3 pr-3 pt-3 ${workstream.parent ? '' : ''}`}>
          <div
            data-testid="workstream-tags"
            className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5 overflow-hidden [&>button]:min-w-0 [&>button]:max-w-full [&>button]:flex-[0_1_auto] [&>button]:overflow-hidden [&>button]:text-ellipsis [&>button]:whitespace-nowrap"
          >
