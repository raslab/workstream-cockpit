...
export function FilterPanel({ filters, onFiltersChange, onClose }: FilterPanelProps) {
  ...
  return (
    <div
      ref={panelRef}
      className="absolute right-0 top-full z-20 mt-1 w-80 rounded-md border border-gray-200 bg-white shadow-lg"
    >
          ...
          {tags && tags.length > 0 ? (
            <>
              ...
              <input
                ...
                className="mb-2 w-full rounded border border-gray-300 px-2 py-1.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
              />
              
              {/* Tag list */}
              <div className="max-h-40 space-y-1 overflow-y-auto">
                {filteredTags.length === 0 ? (
                  <p className="py-2 text-center text-sm text-gray-500">No tags found</p>
                ) : (
          ...
          return (
            <button
              ...
              className={`flex w-full items-center justify-between rounded px-3 py-2 text-left text-sm hover:bg-gray-50 ${
                isActive ? 'bg-gray-50 font-medium text-primary-600' : 'text-gray-700'
              }`}
            >
...
export function GroupMenu({ currentGroup, onGroupChange, onClose }: GroupMenuProps) {
  ...
  return (
    <div
      ref={menuRef}
      className="absolute right-0 top-full z-10 mt-1 w-40 rounded-md border border-gray-200 bg-white shadow-lg"
    >
      <div className="p-1">
