...
const stringProp = (min = 0, max?: number) => ({ type: 'string', ...(min ? { minLength: min } : {}), ...(max ? { maxLength: max } : {}) });
...
const limitProp = (defaultLimit = 100, maximum = 200) => ({ type: 'integer', minimum: 1, maximum, default: defaultLimit });
const colorProp = () => ({ type: 'string', pattern: '^#[0-9A-Fa-f]{6}$' });
const rateLimitBuckets = new Map<string, RateLimitBucket>();
const authFailureBuckets = new Map<string, RateLimitBucket>();

function clean<T>(value: T): T { return JSON.parse(text(value)); }
function ok(data: Record<string, any>) { return { ok: true, ...clean(data) }; }
function limit(args: Record<string, any>, defaultLimit = 100, max = 200) {
  const raw = args.limit ?? defaultLimit;
  if (typeof raw !== 'number' || !Number.isFinite(raw) || !Number.isInteger(raw)) throw new ToolError('limit must be a finite integer');
  if (raw < 1) throw new ToolError('limit must be at least 1');
...
function integerArg(args: Record<string, any>, key: string, min?: number, max?: number) {
  ...
  if (typeof value !== 'number' || !Number.isFinite(value) || !Number.isInteger(value)) throw new ToolError(`${key} must be a finite integer`);
...
function requireString(args: Record<string, any>, key: string, max?: number) {
  if (typeof args[key] !== 'string' || args[key].trim().length === 0) throw new ToolError(`${key} is required`);
...
function optionalString(args: Record<string, any>, key: string, max?: number, nullable = false) {
  ...
  if (max && args[key].length > max) throw new ToolError(`${key} must be ${max} characters or fewer`);
  return args[key];
}
function dateArg(value: unknown, key: string): Date | undefined {
  ...
  if (!new RegExp(ISO_DATE_OR_TIMESTAMP_PATTERN).test(value)) throw new ToolError(`${key} must be a valid ISO date or timestamp`);
...
function authFailureLimit(): number {
  const value = Number(process.env.MCP_AUTH_FAILURE_RATE_LIMIT_PER_MINUTE ?? String(DEFAULT_AUTH_FAILURE_LIMIT_PER_MINUTE));
...
function isAuthFailureRateLimited(req: Request): boolean {
  ...
  const limited = !!bucket && Date.now() - bucket.windowStart < 60_000 && bucket.count >= authFailureLimit();
...
async function authenticate(req: Request): Promise<McpContext | null> {
  ...
  const project = await prisma.project.findFirst({ where: { personId: verified.personId }, orderBy: { createdAt: 'asc' } });
...
function validateValue(value: unknown, propSchema: any, key: string): void {
  if (propSchema?.anyOf) {
    const valid = propSchema.anyOf.some((candidate: any) => {
      try { validateValue(value, candidate, key); return true; } catch { return false; }
    });
  ...
  if (propSchema?.enum) {
    if (!propSchema.enum.includes(value)) throw new ToolError(`${key} must be one of: ${propSchema.enum.join(', ')}`);
    return;
  }
  ...
  switch (propSchema?.type) {
    case 'string':
      ...
      if (propSchema.minLength !== undefined && value.length < propSchema.minLength) throw new ToolError(`${key} must be at least ${propSchema.minLength} characters`);
      if (propSchema.maxLength !== undefined && value.length > propSchema.maxLength) throw new ToolError(`${key} must be ${propSchema.maxLength} characters or fewer`);
      if (propSchema.pattern && !new RegExp(propSchema.pattern).test(value)) throw new ToolError(`${key} is invalid`);
      if (propSchema.format === 'uuid' && !new RegExp(UUID_PATTERN).test(value)) throw new ToolError(`${key} must be a valid UUID`);
      break;
    case 'boolean':
    ...
    case 'integer':
      if (typeof value !== 'number' || !Number.isFinite(value) || !Number.isInteger(value)) throw new ToolError(`${key} must be a finite integer`);
      if (propSchema.minimum !== undefined && value < propSchema.minimum) throw new ToolError(`${key} must be at least ${propSchema.minimum}`);
      if (propSchema.maximum !== undefined && value > propSchema.maximum) throw new ToolError(`${key} must be ${propSchema.maximum} or fewer`);
      break;
    case 'array':
      if (!Array.isArray(value)) throw new ToolError(`${key} must be an array`);
      if (propSchema.maxItems !== undefined && value.length > propSchema.maxItems) throw new ToolError(`${key} must contain ${propSchema.maxItems} items or fewer`);
      value.forEach((item, index) => validateValue(item, propSchema.items ?? {}, `${key}[${index}]`));
      break;
    case 'object':
      if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new ToolError(`${key} must be an object`);
      break;
    case 'null':
...
function validateArgs(tool: ToolDef, args: unknown): Record<string, any> {
  ...
  if (tool.inputSchema?.additionalProperties === false) {
    for (const key of Object.keys(typedArgs)) {
      if (!Object.prototype.hasOwnProperty.call(properties, key)) throw new ToolError(`${key} is not allowed`);
    }
  }
...
function checkRateLimit(ctx: McpContext): boolean {
  ...
  const perMinute = Number(process.env.MCP_RATE_LIMIT_PER_MINUTE ?? String(DEFAULT_RATE_LIMIT_PER_MINUTE));
...
function shouldAudit(tool: ToolDef): boolean {
  return tool.scope === 'mcp:write' || tool.annotations?.destructiveHint || /_(close|reopen|reorder|delete)$/.test(tool.name);
}

function auditTool(ctx: McpContext, tool: ToolDef, args: Record<string, any>, success: boolean): void {
  if (!shouldAudit(tool)) return;
  logger.info(`MCP audit tokenId=${ctx.personalAccessToken.id} personId=${ctx.personId} tool=${tool.name} targetId=${targetIdFromArgs(args) ?? 'none'} success=${success}`);
}
...
function decodeCursor(value: unknown, kind: CursorKind): CursorPayload | undefined {
  ...
  try {
    ...
    if (parsed.v !== 1 || parsed.kind !== kind || typeof parsed.id !== 'string' || !parsed.createdAt) throw new Error('bad cursor');
...
function compareDesc(a: { createdAt: Date | string; id: string }, b: { createdAt: Date | string; id: string }): number {
  const at = new Date(a.createdAt).getTime();
  const bt = new Date(b.createdAt).getTime();
  if (bt !== at) return bt - at;
  return b.id.localeCompare(a.id);
}
function afterCursor<T extends { createdAt: Date | string; id: string }>(items: T[], cursor: CursorPayload | undefined): T[] {
...
function paginate<T extends { createdAt: Date | string; id: string }>(items: T[], pageLimit: number, kind: CursorKind): { page: T[]; nextCursor: string | null } {
  ...
  return {
    page,
    nextCursor: hasMore && last ? encodeCursor({ v: 1, kind, createdAt: new Date(last.createdAt).toISOString(), id: last.id }) : null,
  };
}
...
const tools: ToolDef[] = [
  ...
  { name: 'workstreams_get', description: 'Get a workstream', scope: 'mcp:read', inputSchema: schema({ id: uuidProp(), includeUpdates: { type: 'boolean', default: false }, updatesLimit: limitProp(50, 200) }, ['id']), handler: async (args, ctx) => {
    ...
    const updates = args.includeUpdates ? await prisma.statusUpdate.findMany({ where: { workstreamId: workstream.id }, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }], take: limit({ limit: args.updatesLimit }, 50) }) : [];
    return ok({ workstream, updates });
  }},
  { name: 'workstreams_create', description: 'Create a workstream', scope: 'mcp:write', inputSchema: schema({ name: stringProp(1,200), categoryId: uuidProp(), parentId: uuidProp(), context: stringProp(0,2000), initialStatus: stringProp(0,500), initialNote: stringProp(0,2000) }, ['name']), handler: async (args, ctx) => ok({ workstream: await createWorkstream({ projectId: ctx.projectId, name: requireString(args, 'name', 200), categoryId: optionalString(args, 'categoryId', 100) as any, parentId: optionalString(args, 'parentId', 100) as any, context: optionalString(args, 'context', 2000) as any, initialStatus: optionalString(args, 'initialStatus', 500) as any, initialNote: optionalString(args, 'initialNote', 2000) as any }) }) },
  { name: 'workstreams_update', description: 'Update a workstream', scope: 'mcp:write', inputSchema: schema({ id: uuidProp(), name: stringProp(1,200), categoryId: { anyOf: [uuidProp(), { type: 'null' }] }, parentId: { anyOf: [uuidProp(), { type: 'null' }] }, context: { anyOf: [stringProp(0,2000), { type: 'null' }] } }, ['id']), handler: async (args, ctx) => {
    await assertWorkstream(ctx.projectId, requireString(args, 'id'));
    const data: any = {};
    if (args.name !== undefined) data.name = requireString(args, 'name', 200);
    if (args.categoryId !== undefined) data.categoryId = optionalString(args, 'categoryId', 100, true);
  ...
  { name: 'workstreams_close', description: 'Close a workstream', scope: 'mcp:write', inputSchema: schema({ id: uuidProp() }, ['id']), handler: async (args, ctx) => ok({ workstream: await closeWorkstream(requireString(args, 'id'), ctx.projectId) }) },
  { name: 'workstreams_reopen', description: 'Reopen a workstream', scope: 'mcp:write', inputSchema: schema({ id: uuidProp() }, ['id']), handler: async (args, ctx) => ok({ workstream: await reopenWorkstream(requireString(args, 'id'), ctx.projectId) }) },

  { name: 'updates_list', description: 'List updates for a workstream', scope: 'mcp:read', inputSchema: schema({ workstreamId: uuidProp(), includeSubstreams: { type: 'boolean', default: false }, startDate: dateProp(), endDate: dateProp(), limit: limitProp(), cursor: stringProp(1, 2048) }, ['workstreamId']), handler: async (args, ctx) => {
    await assertWorkstream(ctx.projectId, requireString(args, 'workstreamId'));
    const start = dateArg(args.startDate, 'startDate'); const end = dateArg(args.endDate, 'endDate');
    validateDateRange(start, end);
    const pageLimit = limit(args);
    const cursor = decodeCursor(args.cursor, 'updates_list');
    const createdAt: any = {};
    if (start) createdAt.gte = start; if (end) createdAt.lte = end;
    if (cursor) {
      createdAt.lt = new Date(cursor.createdAt);
    }
    if (args.includeSubstreams) {
      let updates = await getStatusUpdatesByWorkstream(args.workstreamId, { includeSubstreams: true, projectId: ctx.projectId });
      if (start) updates = updates.filter((u: any) => new Date(u.createdAt) >= start!);
      if (end) updates = updates.filter((u: any) => new Date(u.createdAt) <= end!);
      updates = updates.sort(compareDesc);
      const sorted = afterCursor(updates, cursor);
      const { page, nextCursor } = paginate(sorted, pageLimit, 'updates_list');
      return ok({ updates: page, nextCursor });
    }
    const baseWhere: any = { workstreamId: args.workstreamId, ...(start || end || cursor ? { createdAt } : {}) };
    let updates = await prisma.statusUpdate.findMany({ where: baseWhere, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }], take: pageLimit + 1 });
    if (cursor) {
      const sameCreatedAtWhere: any = { workstreamId: args.workstreamId, createdAt: new Date(cursor.createdAt), id: { lt: cursor.id } };
      if (start || end) sameCreatedAtWhere.createdAt = { equals: new Date(cursor.createdAt), ...(start ? { gte: start } : {}), ...(end ? { lte: end } : {}) };
      const sameTime = await prisma.statusUpdate.findMany({ where: sameCreatedAtWhere, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }], take: pageLimit + 1 });
  ...
  { name: 'updates_get', description: 'Get an update', scope: 'mcp:read', inputSchema: schema({ id: uuidProp(), workstreamId: uuidProp() }, ['id','workstreamId']), handler: async (args, ctx) => {
    await assertWorkstream(ctx.projectId, requireString(args, 'workstreamId'));
    const update = await prisma.statusUpdate.findFirst({ where: { id: requireString(args, 'id'), workstreamId: args.workstreamId } });
    if (!update) throw new ToolError('Update not found');
    return ok({ update });
  }},
  { name: 'updates_create', description: 'Create an update', scope: 'mcp:write', inputSchema: schema({ workstreamId: uuidProp(), status: stringProp(1,500), note: stringProp(0,2000) }, ['workstreamId','status']), handler: async (args, ctx) => { await assertWorkstream(ctx.projectId, requireString(args, 'workstreamId')); return ok({ update: await createStatusUpdate({ workstreamId: args.workstreamId, status: requireString(args, 'status', 500), note: optionalString(args, 'note', 2000) as any }) }); }},
  { name: 'updates_update', description: 'Update an update', scope: 'mcp:write', inputSchema: schema({ id: uuidProp(), workstreamId: uuidProp(), status: stringProp(1,500), note: { anyOf: [stringProp(0,2000), { type: 'null' }] } }, ['id','workstreamId']), handler: async (args, ctx) => { await assertWorkstream(ctx.projectId, requireString(args, 'workstreamId')); return ok({ update: await updateStatusUpdate(requireString(args, 'id'), args.workstreamId, { status: optionalString(args, 'status', 500) as any, note: optionalString(args, 'note', 2000, true) as any }) }); }},
  { name: 'updates_delete', description: 'Delete an update', scope: 'mcp:write', inputSchema: schema({ id: uuidProp(), workstreamId: uuidProp(), confirm: { type: 'boolean' } }, ['id','workstreamId','confirm']), annotations: { destructiveHint: true }, handler: async (args, ctx) => { requireConfirm(args); await assertWorkstream(ctx.projectId, requireString(args, 'workstreamId')); const id = requireString(args, 'id'); await deleteStatusUpdate(id, args.workstreamId); return ok({ deletedId: id }); }},

  { name: 'settings_get', description: 'Get settings', scope: 'mcp:read', inputSchema: schema({ include: arrayProp({ enum: ['categories', 'tags', 'views'] }, 3) }), handler: async (args, ctx) => {
    const include = Array.isArray(args.include) && args.include.length ? args.include : ['categories', 'tags', 'views'];
    const data: Record<string, any> = {};
    if (include.includes('categories')) data.categories = await getCategoriesByProjectId(ctx.projectId);
  ...
  { name: 'settings_category_create', description: 'Create category', scope: 'mcp:write', inputSchema: schema({ name: stringProp(1,100), color: colorProp(), emoji: stringProp(0,16), description: stringProp(0,2000) }, ['name','color']), handler: async (args, ctx) => ok({ category: await createCategory({ projectId: ctx.projectId, name: requireString(args, 'name', 100), color: normalizeColor(requireString(args, 'color')), emoji: optionalString(args, 'emoji', 16) as any, description: optionalString(args, 'description', 2000) ?? '' }) }) },
  { name: 'settings_category_update', description: 'Update category', scope: 'mcp:write', inputSchema: schema({ id: uuidProp(), name: stringProp(1,100), color: colorProp(), emoji: { anyOf: [stringProp(0,16), { type: 'null' }] }, description: stringProp(0,2000) }, ['id']), handler: async (args, ctx) => { const data: any = {}; if (args.name !== undefined) data.name = requireString(args, 'name', 100); if (args.color !== undefined) data.color = normalizeColor(requireString(args, 'color')); if (args.emoji !== undefined) data.emoji = optionalString(args, 'emoji', 16, true); if (args.description !== undefined) data.description = optionalString(args, 'description', 2000) ?? ''; return ok({ category: await updateCategory(requireString(args, 'id'), ctx.projectId, data) }); }},
  { name: 'settings_category_delete', description: 'Delete category', scope: 'mcp:write', inputSchema: schema({ id: uuidProp(), confirm: { type: 'boolean' } }, ['id','confirm']), annotations: { destructiveHint: true }, handler: async (args, ctx) => { requireConfirm(args); const id = requireString(args, 'id'); await deleteCategory(id, ctx.projectId); return ok({ deletedId: id }); }},
  { name: 'settings_category_reorder', description: 'Reorder categories', scope: 'mcp:write', inputSchema: schema({ categoryIds: arrayProp(uuidProp(), 200) }, ['categoryIds']), handler: async (args, ctx) => ok({ categories: await reorderCategories(ctx.projectId, args.categoryIds) }) },

  { name: 'settings_tag_create', description: 'Create tag', scope: 'mcp:write', inputSchema: schema({ displayName: stringProp(1,50), color: colorProp() }, ['displayName','color']), handler: async (args, ctx) => ok({ tag: await createTag({ projectId: ctx.projectId, displayName: requireString(args, 'displayName', 50), color: normalizeColor(requireString(args, 'color')) }) }) },
  { name: 'settings_tag_update', description: 'Update tag by UUID', scope: 'mcp:write', inputSchema: schema({ id: uuidProp(), displayName: stringProp(1,50), color: colorProp() }, ['id']), handler: async (args, ctx) => { const data: any = {}; if (args.displayName !== undefined) data.displayName = requireString(args, 'displayName', 50); if (args.color !== undefined) data.color = normalizeColor(requireString(args, 'color')); return ok({ tag: await updateTag(requireString(args, 'id'), ctx.projectId, data) }); }},
  { name: 'settings_tag_delete', description: 'Delete tag by UUID', scope: 'mcp:write', inputSchema: schema({ id: uuidProp(), confirm: { type: 'boolean' } }, ['id','confirm']), annotations: { destructiveHint: true }, handler: async (args, ctx) => { requireConfirm(args); const id = requireString(args, 'id'); await deleteTag(id, ctx.projectId); return ok({ deletedId: id }); }},

  { name: 'settings_views_list', description: 'List views', scope: 'mcp:read', inputSchema: schema(), handler: async (_args, ctx) => ok({ views: await viewService.getProjectViews(ctx.projectId) }) },
  { name: 'settings_view_get', description: 'Get view', scope: 'mcp:read', inputSchema: schema({ id: uuidProp() }, ['id']), handler: async (args, ctx) => ok({ view: await viewService.getView(requireString(args, 'id'), ctx.projectId) }) },
  { name: 'settings_view_create', description: 'Create view', scope: 'mcp:write', inputSchema: schema({ name: stringProp(1,100), isDefault: { type: 'boolean', default: false }, config: { type: 'object' } }, ['name','config']), handler: async (args, ctx) => ok({ view: await viewService.createView(ctx.projectId, { name: requireString(args, 'name', 100), isDefault: Boolean(args.isDefault), config: args.config }) }) },
...
async function handleRpc(body: any, ctx: McpContext) {
  ...
  if (body.method === 'tools/call') {
    ...
    if (!ctx.scopes.includes(tool.scope)) return isNotification ? undefined : error(id, -32001, `Tool requires ${tool.scope} scope`);
    ...
    try {
      ...
      return isNotification ? undefined : result(id, { content: [{ type: 'text', text: structuredContent.summary || 'OK' }], structuredContent });
    } catch (err: any) {
